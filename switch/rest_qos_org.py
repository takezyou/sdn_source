...
REST_ACTION_PUSH_VLAN = 'push_vlan'
REST_ACTION_POP_VLAN = 'pop_vlan'
REST_ACTION_VLAN_ID = 'vlan_id'
REST_ACTION_OUTPUT = 'output'
...
class QoS(object):
    ...
    def _set_qos(self, cookie, rest, waiters, vlan_id):
        ...
        if action is not None:
            ...
            if REST_ACTION_PUSH_VLAN in action:
                actions.append({'type': 'PUSH_VLAN',
                                'ethertype': 33024})
            if REST_ACTION_POP_VLAN in action:
                actions.append({'type': 'POP_VLAN',
                                'ethertype': 33024})
            if REST_ACTION_VLAN_ID in action:
                actions.append({'type': 'SET_FIELD',
                                'field': 'vlan_vid',
                                'value':(int(action[REST_ACTION_VLAN_ID]) | self.dp.ofproto.OFPVID_PRESENT)})
            if REST_ACTION_OUTPUT in action:
                actions.append({'type': 'OUTPUT',
                                'port': action[REST_ACTION_OUTPUT]})
